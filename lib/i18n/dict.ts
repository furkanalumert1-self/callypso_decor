import type { Lang } from "./config";

/**
 * Shared UI strings (the "chrome" — auth, nav, buttons, section labels).
 * App-specific copy lives in app.config.ts instead.
 */
export const ui = {
  tr: {
    // marketing nav
    features: "Özellikler",
    howItWorks: "Nasıl çalışır",
    pricing: "Fiyatlar",
    faq: "S.S.S.",
    signIn: "Giriş yap",
    getStarted: "Başla",
    // auth
    welcomeBack: "Tekrar hoş geldin",
    createAccount: "Hesap oluştur",
    email: "E-posta",
    password: "Şifre",
    fullName: "Ad Soyad",
    continueDemo: "Demo ile devam et",
    orContinueWith: "veya şununla devam et",
    noAccount: "Hesabın yok mu?",
    haveAccount: "Zaten hesabın var mı?",
    demoNote: "Demo modu · Supabase bağlı değil — herhangi bir e-posta/şifreyle girebilirsin.",
    liveNote: "Hesabın Supabase ile doğrulanır. Panel şimdilik örnek verilerle çalışır.",
    checkEmail: "Hesabın oluşturuldu. Giriş yapmadan önce e-postana gelen onay bağlantısına tıkla.",
    authFailed: "Giriş yapılamadı. E-posta ve şifreni kontrol et.",
    authBlurb: "Önceden doldurulmuş, canlı bir demo paneli açmak üzeresin. Etrafına tıkla — hiçbir şey gerçek değil, hiçbir şey bozulmaz.",
    bySigningIn: "Giriş yaparak hizmet şartlarını kabul etmiş olursun.",
    // dashboard chrome
    search: "Ara…",
    recentActivity: "Son hareketler",
    export: "Dışa aktar",
    viewAll: "Tümünü gör",
    welcomeUser: "Tekrar hoş geldin",
    todayOverview: "Bugünün özeti",
    logout: "Çıkış yap",
    account: "Hesap",
    // settings
    settings: "Ayarlar",
    brand: "Marka",
    productName: "Ürün adı",
    domain: "Alan adı",
    tagline: "Slogan",
    integrations: "Entegrasyonlar",
    connected: "Bağlı",
    demoMode: "Demo modu",
    required: "gerekli",
    saveChanges: "Değişiklikleri kaydet",
    brandHint: "Bunlar app.config.ts dosyasından gelir. Değiştirmek için kurulumu tekrar çalıştır.",
    integrationsHint: ".env.local içinde anahtarlar olduğunda bağlanır. Anahtar yoksa o özellik demo modda kalır.",
    fullVersion: "Tam sürümde kullanılabilir",
    connect: "Bağla",
    demoBadge: "Demo modu",
    resetDemo: "Demo verisini sıfırla",
    resetDone: "Demo verisi sıfırlandı",
    menu: "Menü",
    notifications: "Bildirimler",
    markRead: "Tümünü okundu say",
    noNotifications: "Yeni bildirim yok",
    pages: "Sayfalar",
    projectsLabel: "Projeler",
    noResults: "Sonuç bulunamadı",
    guest: "Demo kullanıcı",
    loggedOut: "Çıkış yapıldı",
  },
  en: {
    features: "Features",
    howItWorks: "How it works",
    pricing: "Pricing",
    faq: "FAQ",
    signIn: "Sign in",
    getStarted: "Get started",
    welcomeBack: "Welcome back",
    createAccount: "Create your account",
    email: "Email",
    password: "Password",
    fullName: "Full name",
    continueDemo: "Continue with demo",
    orContinueWith: "or continue with",
    noAccount: "Don't have an account?",
    haveAccount: "Already have an account?",
    demoNote: "Demo mode · Supabase not connected — any email/password works.",
    liveNote: "Your account is verified with Supabase. The dashboard still shows sample data.",
    checkEmail: "Account created. Click the confirmation link in your email before signing in.",
    authFailed: "Couldn't sign in. Check your email and password.",
    authBlurb: "You're about to open a pre-loaded, live demo dashboard. Click around — nothing is real, nothing breaks.",
    bySigningIn: "By signing in you agree to the terms of service.",
    search: "Search…",
    recentActivity: "Recent activity",
    export: "Export",
    viewAll: "View all",
    welcomeUser: "Welcome back",
    todayOverview: "Today's overview",
    logout: "Log out",
    account: "Account",
    settings: "Settings",
    brand: "Brand",
    productName: "Product name",
    domain: "Domain",
    tagline: "Tagline",
    integrations: "Integrations",
    connected: "Connected",
    demoMode: "Demo mode",
    required: "required",
    saveChanges: "Save changes",
    brandHint: "These come from app.config.ts. Re-run setup any time to change them.",
    integrationsHint: "Connected when the keys exist in .env.local. Missing keys keep that feature in demo mode.",
    fullVersion: "Available in full version",
    connect: "Connect",
    demoBadge: "Demo Mode",
    resetDemo: "Reset demo data",
    resetDone: "Demo data reset",
    menu: "Menu",
    notifications: "Notifications",
    markRead: "Mark all as read",
    noNotifications: "No new notifications",
    pages: "Pages",
    projectsLabel: "Projects",
    noResults: "No results",
    guest: "Demo user",
    loggedOut: "Signed out",
  },
} satisfies Record<Lang, Record<string, string>>;

export type UIDict = (typeof ui)["en"];
